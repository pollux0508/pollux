...
import static com.polluxframework.common.util.BaseUtils.checkNextCharIndex;
...
public class FileUtils {
	...
	public static void collectFiles(List<File> collected, File file, List<String> includes) {
		if (file.isFile()) {
			for (String suffix : includes) {
				if (file.getName().endsWith(Constants.DOT + suffix)) {
					collected.add(file);
					break;
				}
			}
	...
	public static char[] getFileLineFeed(final String str) {
		...
		if (BaseUtils.checkNextCharIndex(charArray, 0, windowsLineFeed) != -1) {
			return windowsLineFeed;
		} else if (BaseUtils.checkNextCharIndex(charArray, 0, linuxLineFeed) != -1) {
			return linuxLineFeed;
		} else if (BaseUtils.checkNextCharIndex(charArray, 0, macLineFeed) != -1) {
			return macLineFeed;
		} else {
			return linuxLineFeed;
		}
	...
	public static void clearBlankLines(StringBuilder stringBuilder) {
		...
		while (chasLength >= doubleLineFeedLength) {
			index = BaseUtils.checkNextCharIndex(chas, index, lineFeedChar);
			if (index == -1) {
				return;
			}
			int index2 = BaseUtils.checkNextCharIndex(chas, index + lineFeedLength, lineFeedChar);
			if (index2 == -1) {
				return;
			}
